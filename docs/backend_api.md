...
- **Notification routing**:
  ...
  - Webhook command alias resolution now uses the same canonical command source as websocket mode (`COMMANDS_FILE` / `bot/commands.yml` plus bot-parity defaults), so aliases like `req`, `sr`, `pp`, `undo`, and `del` resolve identically across both ingress paths.
